...
def inicio():
    while True:
        try:
            pergunta = int(input("Digite se gostaria de 1-Escrever (Criar), 2-Ler (abrir) ou 3-Sobrescrever arquivo já escrito?\n"))
    ...
    for i in range(n):
        ...
        for j in range(n):
            if(i==j):
                ...
                while True:
                    try:
                        pergunta=int(input("O vértice %d tem aresta com o vértice %d (responda 1 para sim ou 0 para não)\n" %(i, j) ))
